  ...
  return (
    <section className="py-24 px-6">
      ...
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h2
          className="mb-4 text-[clamp(2.5rem,6vw,4.5rem)] uppercase leading-none tracking-wide"
          style={{ fontFamily: 'var(--font-display)' }}
        >
          ¿Ya encontraste tu foto?
        </h2>
        <p className="mb-10 text-legnar-gray">
        ...
        <a
          ...
          className="group inline-flex items-center justify-center gap-3 rounded-full bg-green-600 px-10 py-4 text-base font-bold uppercase tracking-widest text-white shadow-[0_0_0_0_#16a34a] transition-all duration-300 hover:bg-green-500 hover:shadow-[0_0_32px_#16a34a66]"
        >
          ...
          <div className="flex gap-4">
            {REDES.map((red) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                aria-label={`${red.nombre} ${red.handle}`}
                className="group flex flex-col items-center gap-1.5 rounded-xl border border-legnar-border bg-legnar-dark p-4 text-legnar-gray transition-all duration-300 hover:border-legnar-gold/40 hover:text-legnar-gold"
              >
                <span className="h-5 w-5">{red.icon}</span>
                <span className="text-[10px] tracking-widest">{red.handle}</span>
              </a>
            ))}
          </div>
        </div>
