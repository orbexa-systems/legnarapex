...
export default function GridFotos({ fotos }: { fotos: Foto[] }) {
    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-6 pb-16 pt-2">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 lg:gap-4">
          {fotos.map((foto) => (
            <FotoCard key={foto.id} foto={foto} onOpen={setFotoActiva} />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center gap-6 px-6 py-24 text-center">
      ...
      <a
        ...
        className="inline-flex items-center gap-2 rounded-full bg-green-600 px-6 py-3 text-sm font-bold uppercase tracking-widest text-white transition-all hover:bg-green-500"
      >
